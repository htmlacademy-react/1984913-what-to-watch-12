...
describe(`Reducer: ${ReducerName.SimilarFilms}`, () => {
  ...
  describe('Api action: fetchSimilarFilms', () => {
    it('should update loading status to "true" if is pending', () => {
      expect(similarFilmsData.reducer(state, { type: fetchSimilarFilms.pending.type })).toEqual({ ...state, isSimilarFilmsLoading: true });
    });
    it('should update loading status to "false" and similar films to loaded films if is fullfilled', () => {
      expect(similarFilmsData.reducer(state, { type: fetchSimilarFilms.fulfilled.type, payload: fakeFilms })).toEqual({ ...state, similarFilms: fakeFilms });
    });
    it('should update loading status to "false" and similar films to empty array if an error occured', () => {
      expect(similarFilmsData.reducer(state, { type: fetchSimilarFilms.rejected.type })).toEqual(state);
    });
  });
