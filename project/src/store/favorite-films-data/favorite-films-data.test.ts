...
describe(`Reducer: ${ReducerName.FavoriteFilms}`, () => {
  ...
  describe('Api action: fetchFavoriteFilms', () => {
    it('should update loading status to "true" if is pending', () => {
      expect(favoriteFilmsData.reducer(state, { type: fetchFavoriteFilms.pending.type })).toEqual({ ...state, isFavoriteFilmsLoading: true });
    });
    it('should update loading status to "false" and favorite films to loaded films if is fullfilled', () => {
      expect(favoriteFilmsData.reducer(state, { type: fetchFavoriteFilms.fulfilled.type, payload: fakeFilms })).toEqual({ ...state, films: fakeFilms });
    });
    it('should update loading status to "false" and favorite films to empty array if an error occured', () => {
      expect(favoriteFilmsData.reducer(state, { type: fetchFavoriteFilms.rejected.type })).toEqual(state);
    });
  });

  describe('Api action: postFavoriteFilm', () => {
    it('should update favorite films if film is favorite', () => {
      expect(favoriteFilmsData.reducer(state, { type: postFavoriteFilm.fulfilled.type, payload: fakeFavoriteFilm })).toEqual({ ...state, films: [fakeFavoriteFilm] });
    });
    it('should not update favorite films if film is disliked', () => {
      expect(favoriteFilmsData.reducer(state, { type: postFavoriteFilm.fulfilled.type, payload: fakeDislikedFilm })).toEqual(state);
    });
    it('should not update favorite films if an error occured', () => {
      expect(favoriteFilmsData.reducer(state, { type: postFavoriteFilm.rejected.type })).toEqual(state);
    });
