...
describe(`Reducer: ${ReducerName.Film}`, () => {
  ...
  describe('Api action: fetchFilmById', () => {
    it('should update loading status to "true" if is pending', () => {
      expect(filmData.reducer(state, { type: fetchFilmById.pending.type })).toEqual({ ...state, isFilmLoading: true });
    });
    it('should update loading status to "false" and film to loaded film if is fullfilled', () => {
      expect(filmData.reducer(state, { type: fetchFilmById.fulfilled.type, payload: fakeFilm })).toEqual({ ...state, film: fakeFilm });
    });
    it('should update hasError flag if an error occured', () => {
      expect(filmData.reducer(state, { type: fetchFilmById.rejected.type })).toEqual({ ...state, hasError: true });
    });
  });
