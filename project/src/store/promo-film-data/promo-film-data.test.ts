...
describe(`Reducer: ${ReducerName.PromoFilm}`, () => {
  ...
  describe('Api action: fetchPromoFilm', () => {
    it('should update loading status to "true" if is pending', () => {
      expect(promoFilmData.reducer(state, { type: fetchPromoFilm.pending.type })).toEqual({ ...state, isPromoFilmLoading: true });
    });
    it('should update loading status to "false" and promo film to loaded film if is fullfilled', () => {
      expect(promoFilmData.reducer(state, { type: fetchPromoFilm.fulfilled.type, payload: fakeFilm })).toEqual({ ...state, promoFilm: fakeFilm });
    });
    it('should update loading status to "false" if an error occured', () => {
      expect(promoFilmData.reducer(state, { type: fetchPromoFilm.rejected.type })).toEqual(state);
    });
  });
